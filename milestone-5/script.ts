...
document.addEventListener("DOMContentLoaded", () => {
  ...
  form.addEventListener("submit", (e) => {
    ...
    const brief = (document.querySelector("#brief") as HTMLTextAreaElement)
      .value;
    const profilePic = (
      document.querySelector("#profile-pic") as HTMLInputElement
    ).files![0];

    // Education
    const degrees = document.querySelectorAll(
      ".degree"
    ...
    const skillPercentages = document.querySelectorAll(
      ".skill-percentage"
    ) as NodeListOf<HTMLInputElement>;
    let skillsHtml = "";
    ...
    experiences.forEach((experience) => {
      const companyName = (
        experience.querySelector(".experience-input") as HTMLInputElement
      ).value;
      const position = (
        experience.querySelector(".brief-input") as HTMLTextAreaElement
      ).value;
  ...
  function copyLinkToClipboard(resumeUrl: string) {
    if (resumeUrl) {
      navigator.clipboard
        .writeText(resumeUrl)
        .then(() => {
